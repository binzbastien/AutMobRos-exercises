...

{
    ...
    slSystemOff.setOutputActions({s         et(greenLED, false), set(redLED, false)});
    ...
    slMotorPowerOn.setOutputActions({       set(greenLED, , true), set(buttoredLED, false)});
    ...
    slSystemOn.setLevelAction([&, dt](SafetyContext *privateContext) {
        if (slSystemOn.getNofActivations()*dt >= 1) // wait 1sec
        {
            cs.timedomain.start();
        }
    });
    ...
    slMotorPowerOn.setLevelAction([&, dt](SafetyContext *privateContext){
        if (slMotorPowerOn.getNofActivations()*dt >= 5) //wait 5sec
        {
    ...
    slSystemMoving.setLevelAction([&, dt](SafetyContext *privateContext){
        if (slSystemMoving.getNofActivations()*dt >= 5) //wait 5sec
        {
